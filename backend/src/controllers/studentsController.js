...
const saveChinaLanguageWork = async (client, studentId, body) => {
  if (body.china && body.china.has_experience !== undefined) {
    const c = body.china;
    await client.query(
      `
      INSERT INTO student_china_experience
        (student_id, has_experience, university_name, city, start_date, end_date, program_major)
      VALUES ($1,$2,$3,$4,$5,$6,$7)
      ON CONFLICT (student_id) DO UPDATE
      SET has_experience=$2, university_name=$3, city=$4,
          start_date=$5, end_date=$6, program_major=$7
      `,
      [
        studentId,
        c.has_experience || false,
        n(c.university_name),
        n(c.city),
        n(c.start_date),
        n(c.end_date),
    ...
    for (const l of body.languages) {
      ...
      await client.query(
        ...
        INSERT INTO student_language
          (student_id, language, test_name, score, test_date)
        VALUES ($1,$2,$3,$4,$5)
        `,
        [studentId, n(l.language), n(l.test_name), n(l.score), n(l.test_date)]
      );
    }
  }
    ...
    for (const w of body.work) {
      ...
      await client.query(
        ...
        INSERT INTO student_work_experience
          (student_id, employer, position, start_date, end_date, description)
        VALUES ($1,$2,$3,$4,$5,$6)
        `,
        [
          studentId,
